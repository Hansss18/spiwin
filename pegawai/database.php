...
class database{
	...
	function tampil_data_barang(){
		$data = mysqli_query($this->koneksi, "select barang.kd_brg, barang.nm_brg,suplier.nm_sp,barang.qty,barang.hrg from barang
	...
	function tampil_data_pem(){
		$data = mysqli_query($this->koneksi, "select pembayaran.kd_pmb, pembayaran.tgl,transaksi.kd_tr,barang.nm_brg, barang.hrg, pembeli.nm_pbl,pembayaran.total from pembayaran join transaksi on pembayaran.kd_tr = transaksi.kd_tr join barang on transaksi.kd_brg = barang.kd_brg join pembeli on transaksi.kd_pbl = pembeli.kd_pbl ");
		while($row = mysqli_fetch_array($data)){
			$hasil[] = $row;
		}
		return $hasil;
...
function tanggal(){
	...
	function tambah_data_barang($kd,$nama,$supp,$qty,$harga){
	 	mysqli_query($this->koneksi, "insert into barang values('$kd','$nama','$supp','$qty','$harga')");
	}
    function tambah_data_pembeli($kd,$nama,$no,$alamat){
	 ...
	 function tambah_data_pem($kd,$tgl,$total,$kt){
		mysqli_query($this->koneksi, "insert into pembayaran values('$kd','$tgl','$total','$kt')");
	}
...
function get_by_pem($kd_pem){
	...
	function get_by_struk($kd_pem){
		$query = mysqli_query($this->koneksi,"select pembayaran.kd_pmb, pembayaran.tgl,transaksi.kd_tr,barang.nm_brg, barang.hrg, pembeli.nm_pbl,pembayaran.total from pembayaran join transaksi on pembayaran.kd_tr = transaksi.kd_tr join barang on transaksi.kd_brg = barang.kd_brg join pembeli on transaksi.kd_pbl = pembeli.kd_pbl where kd_pmb='$kd_pem'");
		return $query->fetch_array();
	}
	...
	function update_data_barang($kd,$nama,$supp,$qty,$harga){
        $query = mysqli_query($this->koneksi,"update barang set nm_brg ='$nama', kd_sp ='$supp', qty ='$qty', hrg='$harga' where kd_brg ='$kd'");
	}
